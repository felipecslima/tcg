...
async function fetchRetry(url, retries = 6) {
...
async function ocrVariants(worker, buf, W, H) {
    const results = [];
...
async function main() {
    ...
    for (const t of TARGETS) {
        ...
        else if (auto && !correctId) {
            decision = 'AUTO ✗ (falso-positivo)';
            autoWrong++;
        }
        else {
            decision = 'PENDÊNCIA (manual)';
            pend++;
        }
        console.log(`[${decision}] ${t.id.padEnd(11)} ${t.kind}`);
        console.log(`        leu: ${ex.best ? ex.best.number + (ex.best.denominator ? '/' + ex.best.denominator : '') + ' via ' + ex.best.source : '—'}` +
