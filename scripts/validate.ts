...
import type { ScanInput, MatchStatus } from '../src/matching/index.js';
...
interface Case {
  title: string;
  input: ScanInput;
  expectStatus: MatchStatus;
  expectId?: string; // se definido, exige esse card.id no melhor candidato
}
...
async function main() {
  ...
  for (const c of cases) {
    ...
    console.log(
      `        esperado: ${c.expectStatus}${c.expectId ? ' / ' + c.expectId : ''}` +
        ` | obtido: ${r.status} / ${got} conf=${r.confidence.toFixed(2)}`,
    );
    console.log(`        motivo: ${r.reason}`);
    if (!ok) console.log(`        >>> DIVERGÊNCIA`);
    console.log();
  }
