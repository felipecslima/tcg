...
export interface ExtractCandidate {
  number: string;
  denominator?: string;
  source: string; // qual variante gerou (depuração)
  rawText: string;
}

export interface ExtractResult {
  best: ExtractCandidate | null;
  validatedInSet: boolean; // número existe no set?
  denomMatches: boolean; // denominador bate com printedTotal?
  tried: ExtractCandidate[];
}
 ...
 */
export function pickBestNumber(
  ocrResults: { source: string; rawText: string }[],
  valid: Set<string>,
  printedTotal: number,
): ExtractResult {
  const tried: ExtractCandidate[] = [];
  let inSet: ExtractCandidate | null = null;
  let inSetDenom: ExtractCandidate | null = null;
  let anyParse: ExtractCandidate | null = null;

  for (const r of ocrResults) {
    const p = parseCollectorNumber(r.rawText);
    if (!p) continue;
    const cand: ExtractCandidate = { number: p.number, denominator: p.denominator, source: r.source, rawText: r.rawText };
    tried.push(cand);
    anyParse ??= cand;
