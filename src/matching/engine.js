...
const DEFAULTS = { minConfidence: 0.7, nameMargin: 0.12 };
export class SetIndex {
    ...
    constructor(cards) {
        ...
        for (const c of cards) {
            ...
            if (arr)
                ...
                this.byNumber.set(key, [c]);
        }
    }
    static from(cards) {
        if (!cards.length)
            throw new Error('SetIndex.from: lista de cartas vazia');
    ...
    resolveByNumber(hits, input, opt, usedOcrVariant) {
